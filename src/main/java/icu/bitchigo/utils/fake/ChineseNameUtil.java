...
import icu.bitchigo.utils.lang.RandomUtil;
import icu.bitchigo.utils.lang.StringUtil;

public class ChineseNameUtil {
    private static final String[] singleSur = new String[]{"赵",
    ...
    private static final String[] doubleSur = new String[]{"万俟",
    ...
    private static final String[] ethnicSur = new String[]{"萨嘛喇", "赫舍里", "萨克达", "钮祜禄", "他塔喇", "喜塔腊", "库雅喇", "瓜尔佳",
            "舒穆禄", "索绰络", "依尔觉罗", "额尔德特", "叶赫那拉", "讷殷富察", "叶赫那兰", "爱新觉罗", "依尔根觉罗"};

    private static final String[] chineseWord = new String[]{"阿",
     ...
     * 三字名占比
     */
    public static final int ethnicPercentage = 8;

    public static String randomSurname() {
        int randomSeed = RandomUtil.rangeInt(0, singleSur.length + doubleSur.length + ethnicSur.length);
        //少数民族姓氏
        if (randomSeed >= singleSur.length + doubleSur.length) {
            return ethnicSur[randomSeed - singleSur.length - doubleSur.length];
            //双字名姓氏
        } else if (randomSeed >= singleSur.length) {
            return doubleSur[randomSeed - singleSur.length];
            //单字名姓氏
        } else {
            return singleSur[randomSeed];
        }
    }

    public static String randomPreName() {
        int randomSeed = RandomUtil.rangeInt(0, singlePercentage + doublePercentage + ethnicPercentage);
        //三字名
        if (randomSeed >= singlePercentage + doublePercentage) {
            return StringUtil.buildString(
                    chineseWord[RandomUtil.rangeInt(0, chineseWord.length)],
                    chineseWord[RandomUtil.rangeInt(0, chineseWord.length)],
                    chineseWord[RandomUtil.rangeInt(0, chineseWord.length)]
            );
            //二字名
        } else if (randomSeed >= singlePercentage) {
            return chineseWord[RandomUtil.rangeInt(0, chineseWord.length)] +
                    chineseWord[RandomUtil.rangeInt(0, chineseWord.length)];
            //一字名
        } else {
            return chineseWord[RandomUtil.rangeInt(0, chineseWord.length)];
    ...
    public static String randomName() {
        return randomSurname() + randomPreName();
    }
